...
exports.ringUser = ringUser;
exports.ringWebUser = ringWebUser;
exports.initiateWebCall = initiateWebCall;
...
const WhisperTranscriptionService_1 = __importDefault(require("../services/WhisperTranscriptionService"));
 ...
 */
async function ringUser(recipientId, callerId, callId, callerName) {
    ...
    const recipient = clientManager_1.clientManager.getUserFromDirectory(recipientId);
    if (!recipient) {
    ...
    if (recipient.fcmToken) {
        try {
            await (0, firebaseService_1.sendPushNotification)(recipient.fcmToken, callerId, callerName, callId);
 ...
 */
async function ringWebUser(recipientId, callerId, callId, callerName) {
    logger.info({ callerId, recipientId, callId }, 'Attempting to ring WEB user');
    const webRecipient = clientManager_1.clientManager.getWebUser(recipientId);
    if (!webRecipient) {
...
async function initiateWebCall(socket, data) {
...
function setupSocketIOServer(io) {
    io.on('connection', (socket) => {
        ...
        socket.on('register-user', async (data) => {
            ...
            if (source === 'web') {
                ...
                clientManager_1.clientManager.addWebUser(userId, displayName, socket);
                logger.info({ userId, displayName }, 'Web client registered');
            }
            else {
                // Default to mobile user
                clientManager_1.clientManager.addMobileUser(userId, displayName, socket, fcmToken);
        ...
        socket.on('get-online-users', () => {
            const users = clientManager_1.clientManager.getAllUsers(); // Returns all users with online status
        ...
        */
        socket.on('call-user', async (data) => {
            ...
            try {
                ...
                const permissions = await PermissionService_1.default.getUserPermissions(to);
                ...
                if (permissions.calls === true) {
                    ...
                    // Construct dynamic agent URL for the client
                    // Base URL from config + query params for context
                    const baseHandlerUrl = config_1.config.agentServerUrl;
                    // Construct callback URL for escalation (prefer PUBLIC_URL from env, else fallback to local)
                    const publicUrl = process.env.PUBLIC_URL || `http://${process.env.HOST || '192.168.1.8'}:${config_1.config.port}`;
        ...
        socket.on('call-user-web', async (data) => {
            await initiateWebCall(socket, data);
        });
        // Mobile App - WebRTC Offer
        socket.on('webrtc-offer', async (data, ack) => {
            ...
            const recipient = clientManager_1.clientManager.getUserFromDirectory(to);
            if (!recipient) {
            ...
            if (!recipient.socket) {
                ...
                if (ack)
                    ack({ success: false, error: 'Recipient offline' });
                return;
            }
        ...
        socket.on('webrtc-answer', async (data, ack) => {
        ...
        socket.on('call-accepted', async (data) => {
            ...
            const recipient = clientManager_1.clientManager.getMobileUser(to);
            if (recipient?.socket) {
                recipient.socket.emit('call-accepted', { from, callId });
            }
            // Sync: If user answered on one device, stop ringing on their other devices
            try {
                const mobileUser = clientManager_1.clientManager.getMobileUser(from);
                const webUser = clientManager_1.clientManager.getWebUser(from);
        ...
        socket.on('webrtc-ice-candidate', async (data, ack) => {
        ...
        socket.on('end-call', async (data) => {
            ...
            WhisperTranscriptionService_1.default.stopCall(callId).catch(err => {
        ...
        socket.on('send-message', async (data, ack) => {
            ...
            if (recipient.socket) {
                ...
                if (ack)
                    ack({ success: true, delivered: 'socket' });
            }
            else {
                ...
                const senderUser = clientManager_1.clientManager.getUserFromDirectory(from);
                await UpdatesService_1.default.createUpdate({
                    userId: to,
                    type: 'missed-message',
                    title: 'New message',
        ...
        socket.on('p2p-audio', (data) => {
            if (!userId)
                return;
            WhisperTranscriptionService_1.default.addAudioChunk(data.callId, userId, data.otherUserId, data.audioData);
        });
        // Disconnect
        socket.on('disconnect', () => {
            if (userId) {
                clientManager_1.clientManager.removeMobileUser(userId);
