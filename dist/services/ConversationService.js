...
class ConversationService {
     ...
     */
    async logMessage(senderId, recipientId, text, timestamp) {
        if (!this.db) {
        ...
        try {
            ...
            await this.db.collection('conversations').updateOne({ conversationId }, {
                $setOnInsert: {
                    conversationId,
                    createdAt: now,
                    participants,
                ...
                $push: {
                    messages: message,
                }, // Type cast needed for MongoDB push operation
            }, { upsert: true });
            console.log(`[ConversationService] Message logged: ${conversationId} | ${senderId} → ${recipientId}`);
        }
        catch (error) {
     ...
     */
    async logCallTranscript(senderId, recipientId, text, timestamp) {
        if (!this.db) {
            throw new Error('ConversationService not initialized');
        }
        try {
            ...
            console.log(`[ConversationService] Call transcript logged: ${conversationId} | ${senderId} → ${recipientId}`);
        }
        catch (error) {
